...
const monsterImageUrl = () => {
	if (!props.monster?.image_url) return ''

	const trimmedPath = String(props.monster.image_url).replace(/^\/+/, '')
	const path = `${assetsUrl}/${trimmedPath}`
	const params = `?w=80&h=80&auto=compress&format=webp`
...
const getDropIconUrl = drop => {
	...
	const trimmedPath = String(drop.icon_url).replace(/^\/+/, '')
	const path = `${assetsUrl}/${trimmedPath}`
	const params = `?w=24&h=24&auto=compress&format=webp`
...
const getDropRate = drop => {
	if (typeof drop.rate === 'number' && drop.rate > 0) {
		return drop.rate % 1 === 0
			? `${drop.rate}%`
			: `${parseFloat(drop.rate.toFixed(2))}%`
	}
	...
	if (typeof drop.name === 'string') {
		let match =
			drop.name.match(/([\d.]+%)\s*$/) || drop.name.match(/\(\s*([\d.]+%)\s*\)/)
...
const officialDescriptionLines = computed(() => {
	...
	return item.description.official
		.split('。')
		.map(line => line.trim())
	...
	<div class="monster-card-new">
		<div class="card-top">
			<div class="map-icon-container">
				<img
					:src="`${baseUrl}images/ui/map.png`"
					class="map-icon" />
				<img
					:src="`${baseUrl}images/ui/location.png`"
					class="location-icon" />
				<div
					class="spawn-tooltip"
					v-if="monster.spawns && monster.spawns.length > 0">
					<div
						v-for="spawn in monster.spawns"
						:key="spawn.map_name + spawn.description"
						class="tooltip-item">
						{{ spawn.description.replace('-', '').trim() }} ({{
							spawn.map_name
						}}) {{ spawn.spawn_info || '' }}
					</div>
				</div>
			...
			<div class="attributes">
				...
				<span>
					{{ monster.basic_info.element.type }}屬性{{
						monster.basic_info.element.level
					}}
				</span>
		...
		<div class="identity">
			<img
				...
				height="80"
				:src="monsterImageUrl()"
				:alt="`RO 怪物「${monster.name.zh_tw}」的圖片與掉落資訊 (種族:${monster.basic_info.race})`" />
      ...
      <div class="stat-row">
			...
			<div class="stat-row">
				...
				<span class="stat-value">{{ monster.stats.magic_defense }}</span>
			</div>
			<div class="stat-row">
		...
		<div
			class="drops-list"
			v-if="monster.drops && monster.drops.length > 0">
			<template
				v-for="drop in monster.drops"
				:key="`${monster.id}-${drop.item_id}`">
				<div
					class="drop-item-row clickable"
					@click="toggleItemDetails(drop)">
					<img
						:src="getDropIconUrl(drop)"
						:alt="`RO 怪物「${monster.name.zh_tw}」掉落物品「${getDropName(
							drop
						)}」的圖片`"
						loading="lazy"
						class="drop-icon" />
				...
				<Transition name="expand">
					<div
						...
						v-if="expandedItemId === drop.item_id">
						<div
							v-if="!itemsDatabase"
						...
						<div
							v-else-if="itemsDatabase[drop.item_id]"
							class="details-content">
							<div class="details-basic-info">
								<span>類型: {{ itemsDatabase[drop.item_id].category }}</span>
								<span v-if="itemsDatabase[drop.item_id].attributes.weight">
									重量: {{ itemsDatabase[drop.item_id].attributes.weight }}
								</span>
								<span v-if="drop.slot && drop.slot !== '--'">
									規格: {{ drop.slot }}
								</span>
							</div>
							...
							<div
								...
								v-if="
									itemsDatabase[drop.item_id].description.official &&
									itemsDatabase[drop.item_id].description.official.trim() !==
										'- 沒有 -'
								">
								<div
									v-for="(line, index) in officialDescriptionLines"
									:key="index"
									class="effect-item">
									{{ line }}。
								</div>
							...
							<div
								class="effects-section"
								v-else-if="formattedEffects.length > 0">
								<div
									v-for="effect in formattedEffects"
									:key="effect"
									class="effect-item">
									{{ effect }}
								</div>
